...
function plotKface(grid,flow,blocks,data_type,k_val)


clr = ['k' 'm' 'c' 'y' 'g']; clr = [clr clr clr clr clr clr clr];
skip = 1;
N = 3600; % WARNING!!! Hard coded for 60 Hz MHI turbine 
...
for i = 1:length(blocks),
    ...
    % i = i_val face
    pcolor(squeeze(x(1:skip:im,1:skip:jm,k_val)), ...
    squeeze(r(1:skip:im,1:skip:jm,k_val)), ...
    squeeze(var(1:skip:im,1:skip:jm,k_val)))
