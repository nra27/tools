...
function plotJFace(grid,flow,data_type,blocks,j_val,i_val)

clr = ['.-k'; '.-m' ;'.-c' ;'.-y' ;'.-g'];
clr = [clr; clr;];clr = [clr; clr;];clr = [clr; clr;];clr = [clr; clr;];
clr = [clr; clr;];clr = [clr; clr;];clr = [clr; clr;];clr = [clr; clr;];

N = 3000; % WARNING!!! Hard coded for 50 Hz MHI turbine wheat99
...
for i = 1:length(blocks),
    ...
    % Plot r versus var on the j = jm face
    plot(squeeze(360/(2*pi)*t(i_val,j_val,[1:km])), ...
            squeeze(var(i_val,j_val,[1:km])),clr(n,:))
