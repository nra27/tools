function [data] = Calculate_Taw_STEP
...
%
cd /users/hydra/RT27a/3D/stage_calcs/STAGE_STEP

grid_file = 'STAGE_HT_STEP.grid.1.adf'
flow_file = 'STAGE_HT_STEP.flowuns.adf'
...
for k = 1:100,
    ...
    if k == 1, % We only need to find the nodes on the first time step
        ...
        % Only the HPB
        casing_surfs = casing_surfs(4:8);
        
        %
        % Find the blade surfaces
        %
        disp('Finding the Blade surfaces')
        n = 1;
        for i = 1:max(size(surface_data.surface_groups)),
            temp = surface_data.surface_groups{i};
            if strcmp(temp,'MERGEOGV'),
                blade_surfs(n) = i;
                n = n+1;
            else
            end
    ...
    for i = 1:length(casing_surfs),
        ...
        casing.rho(:,i) = flow_data_318.flow(casing_nodes(:,i),1);
        casing.p(:,i) = flow_data_318.flow(casing_nodes(:,i),5);
    ...
    for i = 1:length(blade_surfs),
        ...
        blade.q(:,i) = q_dot_318;
        blade.rho(:,i) = flow_data_318.flow(blade_nodes(:,i),5);
        blade.p(:,i) = flow_data_318.flow(blade_nodes(:,i),1);
