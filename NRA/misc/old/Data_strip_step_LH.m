...
for k = [2:2:32 36:2:64],

    k 
    
    load(['D:\hydra\grid_2.2.step\post\ADF\ADF_coords_LH_' num2str(k) '.mat'])
    ...
    % Coordinates
    data.plane(k).LH.x = x;    
    data.plane(k).LH.y = z; % Swap from padram to adf  coordinate systems
    data.plane(k).LH.z = y; % Swap from padram to adf  coordinate systems
    data.plane(k).LH.r = sqrt(z.^2+y.^2);
    data.plane(k).LH.theta = atan2(y,z);
