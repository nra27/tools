...
% % Plot NGV annulus profile
% casing = E.n_sections;
% plot(E.section(casing).streamline(:,1),E.section(casing).streamline(:,2),'m.-')
% hold on
% plot(E.section(casing-1).streamline(:,1),E.section(casing-1).streamline(:,2),'m.-')
% plot(E.section(1).streamline(:,1),E.section(1).streamline(:,2),'m.-')
% 
F = Read_Blade_File([root '/HPB/padram/Blade_Definition_HPB_short_NRA_para.dat']);
% Plot HPB annulus profile
casing = F.n_sections;
plot(F.section(casing).streamline(:,1),F.section(casing).streamline(:,2),'.-r')
hold on
%plot(F.section(casing-1).streamline(:,1),F.section(casing-1).streamline(:,2),'.-r')
plot(F.section(1).streamline(:,1),F.section(1).streamline(:,2),'.-r')
