...
% Windows root
WIN_ROOT = 'H:\RT27a\3D\HPB\grid_2.2.step';
...
% Set the file names
% Windows
flow_file = [WIN_ROOT '\HPB_step.flow.adf'];
grid_file = [WIN_ROOT '\grid_files\HPB_step.grid.1.adf'];

% % LINUX 
% flow_file = [LIN_ROOT '/HPB_step.flow.adf'];
% grid_file = [LIN_ROOT '/grid_files/HPB_step.grid.1.adf'];
...
load([WIN_ROOT '\318_933\HPB_step.318_933.flow.mat'])
warning on 
...
eval(['q_dot_step_' num2str(i) '= [q_dot_spline; q_dot_spline; q_dot_spline;];'])
...
eval(['q_dot_plain_' num2str(i) '= [q_dot_spline; q_dot_spline; q_dot_spline;];'])
...
for n = 1:8,
    ...
    % CFD
    eval(['h = plot(x_' num2str(n) '/0.277*360/(2*pi)+1.1,q_dot_step_' num2str(n) ');'])
    set(h,'color','red','linewidth',2)
    
    hold on
    eval(['h = plot(x_' num2str(n) '/0.277*360/(2*pi)+1.1,q_dot_plain_' num2str(n) ');'])
