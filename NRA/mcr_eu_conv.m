...
% Sussex University TFMRC Multiple Cavity Rig - NEWAC Build 2010/2011
%
% [MP,r, prt,NI,TC,Px,rpm,m250,m110,Ro,Re_z,Re_phi,Gr_b_,t,m_bore,m_rad] = ...
% mcr_eu_conv(run)
%

function [MP,r, prt,NI,TC,Px,rpm,m250,m110,Ro,Re_z,Re_phi,Gr_b,t,m_bore,m_rad,T_bore] = mcr_eu_conv(run)
...
%   IDLE   MTO  
bore_mass = [0.29       1.97]; % [ks s^-1]
p_bore =    [118.104e3  795.123e3]; % [Pa]
T_bore =    [344.5      657.7]; % [K]
T_shroud =  [443        807]; % [K]
...
% Grashof b
Gr_b_T1000 = (rho_bore.^2.*omega.^2 * b * (b - a)^3 .* beta .* DT_sh) ./ mu_bore .^2;
Gr_b_T1000 =Gr_b_T1000(1)*(1-IDLE_MTO)+Gr_b_T1000(2)*(IDLE_MTO);
Gr_b_= Gr_b./Gr_b_T1000;

% Grashof sh
Gr_sh_T1000 = (rho_bore.^2.*omega.^2 * b * (s/2)^3 .*beta .* DT_sh) ./ mu_bore .^2;
