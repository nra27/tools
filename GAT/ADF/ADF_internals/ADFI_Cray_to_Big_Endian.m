function [D,to_data,error_return] = ADFI_Cray_to_Big_Endian(from_format,from_os_size,to_format,to_os_size,data_type,delta_from_bytes,delta_to_bytes,from_data,D);
%
% [D,to_data,error_return] = ADFI_Cray_to_Big_Endian(from_format,from_os_size,to_format,to_os_size,data_type,delta_from_bytes,delta_to_bytes,from_data,D)
%
% D - Delcaration space
...
switch data_type
    ...
    case {'I4' 'U4'}
        to_data(1:4) = from_data(5:8);
        
    case {'I8' 'U8'}
        for i = 1:delta_to_bytes
            to_data(i) = from_data(9-delta_to_bytes+i);
        end
    ...
    case 'R4'
        ...
        to_data(1) = bitshift(bitor(exp,127),-1);
        if bitand(exp,1) == 1 % LSB of the exponent
            to_data(2) = bitor(double(to_data(2)),128);
        end
        ...
        to_data(2) = bitor(double(to_data(2)),bitand(double(from_data(3)),127));
        to_data(3) = from_data(4);
        to_data(4) = from_data(5);
    ...
    case 'R8'
        ...
        to_data(2) = bitor(double(to_data(2)),bitshift(bitand(double(from_data(3)),120),5));
        for i = 3:7
