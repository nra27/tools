function [D,to_data,error_return] = ADFI_Big_Endian_to_Cray(from_format,from_os_size,to_format,to_os_size,data_type,delta_from_bytes,delta_to_bytes,from_data,D);
%
% [D,to_data,error_return] = ADFI_Big_Endian_to_Cray(from_format,from_os_size,to_format,to_os_size,data_type,delta_from_bytes,delta_to_bytes,from_data,D)
%
% D - Delcaration space
...
switch data_type
    ...
    case 'I8'
        ...
        for i = 1:delta_from_bytes
            to_data(9-delta_from_bytes+i) = from_data(i);
        end
    ...
    case 'R4'
        ...
        exp = bitshift(bitand(double(from_data(1)),63),1);
          
        if bitand(double(from_data(1),128)) == 128 % Set sign
            exp = exp + 1;
        end
        ...
        to_data(3) = bitor(double(from_data(2)),128);
        to_data(4) = from_data(3);
        to_data(5) = from_data(4);
    ...
    case 'R8'
        ...
        % Cray exponent is 2 greater than the Iris
        exp = bitshift(bitand(double(from_data(1)),63),4)+bitand(bitshift(double(from_data(2)),-4),15);
          
        if bitand(double(from_data(1),16)) == 0 % Set sign
            exp = exp - 1024;
        end
        exp = exp + 2;
        ...
        to_data(3) = bitor(bitor(bitand(bitshift(from_data(2),3),120),bitand(bitshift(from_data(3),-5),7)),128);
        for i = 4:8
            to_data(i) = bitor(bitand(bitshift(from_data(i-1),3),248),bitand(bitshift(from_data(i),-5),7));
        end
        
    case 'X4'
        [D,to_data,error_return] = ADFI_Big_Endian_to_Cray(from_format,from_os_size,to_format,to_os_size,'R4',delta_from_bytes,delta_to_bytes,from_data,D);
        if error_return ~= -1
            return
        end
        
        [D,to_data(9:16),error_return] = ADFI_Big_Endian_to_Cray(from_format,from_os_size,to_format,to_os_size,'R4',delta_from_bytes,delta_to_bytes,from_data(5:8),D);
        if error_return ~= -1
            return
        end
        
    case 'X8'
        [D,to_data,error_return] = ADFI_Big_Endian_to_Cray(from_format,from_os_size,to_format,to_os_size,'R8',delta_from_bytes,delta_to_bytes,from_data,D);
        if error_return ~= -1
            return
        end
        
        [D,to_data(9:16),error_return] = ADFI_Big_Endian_to_Cray(from_format,from_os_size,to_format,to_os_size,'R8',delta_from_bytes,delta_to_bytes,from_data(9:16),D);
        if error_return ~= -1
            return
        end
