function M = Casing_Movie_Comp(Data1,Data2,data_type);
%
% M = Casing_Movie_Comp(Data1,Data2,data_type)
%
...
[time_points,data_points] = size(Data1);

for i = 1:8
    for j = 1:data_points/8
        Plo1(i,j,:) = Mov1(:,i+(j-1)*8);
        Plo2(i,j,:) = Mov2(:,i+(j-1)*8);
    end
end
...
figure(1)
delta1 = 0;
set(1,'position',[160 425 800 380])
set(1,'DoubleBuffer','on','color',[1 1 1]);
a(1) = subplot(1,2,1);
set(a(1),'position',[0.08 0.11 0.4020 0.8150]);
a(2) = subplot(1,2,2);
set(a(2),'position',[0.556 0.11 0.4020 0.8150]);

% Loop through all time steps
for time = [100:144 1:99]
    % Plot the data
    if strcmp(data_type,'HT')
        axes(a(1))
        contourf(data_y,data_x,Plo1(:,:,time),linspace(-0.5e5,2.5e5,14));
        caxis([-5e4 25e4])
        axes(a(2))
        contourf(data_y,data_x,Plo2(:,:,time),linspace(-0.5e5,2.5e5,14));
        caxis([-5e4 25e4])
    elseif strcmp(data_type,'Pres')
        axes(a(1))
        contourf(data_y,data_x,Plo1(:,:,time),linspace(2e5,4.75e5,11));
        caxis([2e5 4.75e5])
        axes(a(2))
        contourf(data_y,data_x,Plo2(:,:,time),linspace(2e5,4.75e5,11));
        caxis([2e5 4.75e5])
    else
        error('unknown call')
    end
    ...
    % Set visual box
    set(a(1),'XLim',[-60 0]);
    set(a(1),'YLim',[-26.9216 19.8796]);
    set(a(2),'XLim',[-60 0]);
    set(a(2),'YLim',[-26.9216 19.8796]);
    
    H = axes('position',[0.275 0.15 0.5 0.02]);
    C = colorbar(H);
    if strcmp(data_type,'HT')
        b = xlabel('Wm^{-2}');
        set(b,'fontsize',14)
        set(C,'XLim',[-5e4 25e4],'fontsize',14)
        child = get(C,'Children');
        set(child,'XData',[-5e4 25e4])
    elseif strcmp(data_type,'Pres')
        b = xlabel('Pa');
        set(b,'fontsize',14)
        set(C,'XLim',[2e5 4.75e5],'fontsize',14)
        child = get(C,'Children');
        set(child,'XData',[2e5 4.75e5])
    end
    
    M(time) = getframe(1,[0 0 800 380]);
