function M = Casing_Snapshot(Data,data_type);
%
% M = Casing_Snapshot(Data,data_type)
%
...
for time = [52 82 112 142 172 202 232 262]-0 %-20 for build1
    % Plot the data
    figure
    ...
    elseif strcmp(data_type,'Pres')
        contourf(data_y,data_x,Plo(:,:,time)*1e5,linspace(2e5,5e5,14));
        caxis([2e5 5e5])
...
elseif strcmp(data_type,'HT')
    ...
    %set(H,'xtick',[2 2.5 3 3.5 4 4.5 5]*1e5)
    caxis([-5e4 25e4])
    set(get(H,'children'),'xdata',[-5e4 25e4])
elseif strcmp(data_type,'H')
    Xlabel('W/m^2K')
    caxis([1e3 4.5e3])
elseif strcmp(data_type,'Nu')
