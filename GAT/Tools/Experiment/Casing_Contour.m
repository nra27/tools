function M = Casing_Contour(Data1,Data2,type);
%
% M = Casing_Contour(Data1,Data2)
%
% A function to create a casing contour
...
for j = 1:8
    data_y(j,:) = linspace(0,-100,7)/100*272*2*pi/36-272*2*pi/36/48*(j-1);
end

% Set up the data array
for i = 1:7
    for j = 1:8
        data1(j,i) = Data1((i-1)*8+j);
        data2(j,i) = Data2((i-1)*8+j);
    end
end
...
pcolor(data_y,data_x,data1)
shading interp
if strcmp(type,'HT')
    caxis([0 16E4])
elseif strcmp(type,'Pres')
    caxis([0 5e5])
elseif strcmp(type,'Tad')
    caxis([320 370])
...
t1 = text(-1.7*272*pi/36,-1*24.35,'Plain Annulus');
set(t1,'Fontsize',12,'fontname','times')
der = -2;

% Plot HPBs
plot(HPB(:,2)+der,HPB(:,1),'b')
plot(HPB(:,2)+der-D_HPB,HPB(:,1),'b')
plot(HPB(:,2)+der-2*D_HPB,HPB(:,1),'b')

% Plot the gauge points
plot(data_y,data_x,'k+')
...
axis equal
hold on
t2 = text(-1.7*272*pi/36,-1*24.35,'Stepped Annulus');
set(t2,'Fontsize',12,'fontname','times')
...
set(h,'position',posh)
if strcmp(type,'HT')
    set(get(h,'Xlabel'),'string','Heat-transfer rate (W/m^2)','fontsize',12,'fontname','times')
elseif strcmp(type,'Pres')
    set(get(h,'Xlabel'),'string','Static pressure (Pa)','fontsize',12,'fontname','times')
elseif strcmp(type,'Tad')
    set(get(h,'Xlabel'),'string','Adiabatic wall temperature (^oC)','fontsize',12,'fontname','times')
elseif strcmp(type,'H')
    set(get(h,'Xlabel'),'string','Heat-transfer coefficient (W/m^2K)','fontsize',12,'fontname','times')
elseif strcmp(type,'Nu')
    set(get(h,'Xlabel'),'string','Nusselt number','fontsize',12,'fontname','times')
end
