function M = Casing_Movie(Data,data_type);
%
% M = Casing_Movie(Data,data_type)
%
...
% Upsample circumferential data
% for i = 0:7
%     Data_new(:,[1:8:441]+i) = interp1([1:8:49],Data(:,[1:8:49]+i)',[1:8:441]/9,'linear')';
% end

% Add phase to the data
Mov = [Data; Data];
% Mov(:,[2 9]) = [Mov((1*5+1):end,[2 9]); Mov(1:(1*5),[2 9])];
% Mov(:,[3 10 17]) = [Mov((2*5+1):end,[3 10 17]); Mov(1:(2*5),[3 10 17])];
% Mov(:,[4 11 18 25]) = [Mov((3*5+1):end,[4 11 18 25]); Mov(1:(3*5),[4 11 18 25])];
% Mov(:,[5 12 19 26 33]) = [Mov((4*5+1):end,[5 12 19 26 33]); Mov(1:(4*5),[5 12 19 26 33])];
% Mov(:,[6 13 20 27 34 40]) = [Mov((5*5+1):end,[6 13 20 27 34 40]); Mov(1:(5*5),[6 13 20 27 34 40])];
% Mov(:,[7 14 21 28 35 41 49]) = [Mov((6*5+1):end,[7 14 21 28 35 41 49]); Mov(1:(6*5),[7 14 21 28 35 41 49])];
% for i = 7:49
%     Mov(:,[8 15 22 29 36 42 50 57]+8*(i-7)) = [Mov((i*5+1):end,[8 15 22 29 36 42 50 57]+8*(i-7)); Mov(1:(i*5),[8 15 22 29 36 42 50 57]+8*(i-7))];
% end
% Mov(:,[400 407 414 421 428 435 442]) = [Mov((50*5+1):end,[400 407 414 421 428 435 442]); Mov(1:(50*5),[400 407 414 421 428 435 442])];
% Mov(:,[408 415 422 429 436 443]) = [Mov((51*5+1):end,[408 415 422 429 436 443]); Mov(1:(51*5),[408 415 422 429 436 443])];
% Mov(:,[416 423 430 437 444]) = [Mov((52*5+1):end,[416 423 430 437 444]); Mov(1:(52*5),[416 423 430 437 444])];
% Mov(:,[424 431 438 445]) = [Mov((53*5+1):end,[424 431 438 445]); Mov(1:(53*5),[424 431 438 445])];
% Mov(:,[432 439 446]) = [Mov((54*5+1):end,[432 439 446]); Mov(1:(54*5),[432 439 446])];
% Mov(:,[440 447]) = [Mov((55*5+1):end,[440 447]); Mov(1:(55*5),[440 447])];
% Mov(:,[448]) = [Mov((56*5+1):end,[448]); Mov(1:(56*5),[448])];
...
for time = [1:144]
    ...
    if strcmp(data_type,'HT')
        ...
        %shading interp
        contourf(data_y,data_x,Plo(:,:,time),linspace(-0.5e5,2.5e5,14));
        caxis([-5e4 25e4])
    elseif strcmp(data_type,'Pres')
        contourf(data_y,data_x,Plo(:,:,time)*1e5,linspace(2e5,4.45e5,11));
        caxis([2e5 4.5e5])
    elseif strcmp(data_type,'Tad')
        contourf(data_y,data_x,Plo(:,:,time),[280:10:400]);
        caxis([280 400])
    elseif strcmp(data_type,'Nu')
        contourf(data_y,data_x,Plo(:,:,time),linspace(1000,3000,10));
        caxis([1000 3000])
    elseif strcmp(data_type,'H');
        contourf(data_y,data_x,Plo(:,:,time),linspace(1e3,4.5e3,11));
        caxis([1e3 4.5e3])
    else
        error('unknown call')
    end
    ...
    H = colorbar('vert');
    set(H,'position',[0.8740 0.25 0.0215 0.3869])
    axes(H)
