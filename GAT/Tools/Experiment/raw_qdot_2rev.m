...
for f=1:file_num;
  ...
  eval(['load ' file(f,1:12) 'subtract' file(f,14:17) '_vol'])
  eval(['load ' file(f,1:12) 'temp' file(f,14:17)])
...
%%%calculate Re number %%%%
  ...
  % Time mean wall temperature
  eval(['Tw(f,1:16)=mean(temp' file(f,14:17) '(pointf1:pointf2,1:16))+273.15;'])   
  ...
  %change time vector to fit design speed
   ...
   eval(['Q' file(f,14:17) '=subtract' file(f,14:17) '_vol(passing(1):passing(lenpass),1:16);'])
   ...
   del_p = 27;
   %load blade_HTR
   eval(['Q' file(f,14:17) '=[Q' file(f,14:17) '(del_p+1:L,:);Q' file(f,14:17) '(1:del_p,:)];'])    

   
      %clear data        
       eval(['clear temp' file(f,14:17)])
       eval(['clear subtract' file(f,14:17) '_vol'])
  
      eval(['line' num2str(f) '_qdot = Q' file(f,14:17) ';']);
      eval(['line' num2str(f) '_pass = t_vec' file(f,14:17) ';']);
      
      save_string1 = [save_string1 ' line' num2str(f) '_qdot'];
...
%   line7_pass=t_vec7773;
  ...
  eval(['delete(''' file(f,1:12) 'rawQdot_2rev.mat'')']);
  eval(['save ' file(f,1:12) 'rawQdot_2rev' save_string1 save_string2 ' Tw opt']);
