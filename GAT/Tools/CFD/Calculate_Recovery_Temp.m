...
for ig = 1:length(viscous_groups)
	...
	Trec = zeros(1,num_boundary_nodes);
	...
	for ib = 1:num_boundary_nodes
		...
		if isempty(free_nodes)
			...
			for ifr = 1:length(free_nodes)
				Ttemp(ifr) = T(partner_nodes(ifr))*(1+Rc*0.5*(gamma-1)*(M(partner_nodes(ifr))^2));
			end
		end
...
%		set(Handles.line2,'xdata',[0 ib/num_boundary_nodes]);
		Trec(ib) = mean(Ttemp);
		clear Ttemp;
	end

	% Reinsert Taw in qdot data.
	surface_data.group(tg).wall_heat_flux = Trec';
	clear Trec;
