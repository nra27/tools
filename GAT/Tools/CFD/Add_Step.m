function Add_Step(mesh_file,boundata_file,inlet_rad_factor,RG_factor)
%
% Add_Step(mesh_file,boundata_file,inlet_rad_factor,RG_factor)
%
% A function to add the upstream step to a suitable
% plot3d mesh.  No checks are done.
%
% Modified by NRA 28-02-2007
%
% ######
% inlet_rad_factor is 0 to put the step on the original casing line
% 
% e.g. 0.0005 lines the step up for the short HPB/NGV
%
% ######
% RG_factor varies the height of the step relative to the tip gap
% NB This is very non-linear as it works on grid lines not the actual height
% 
% e.g. RG_factor = 2 gives a step twice the height of the tip gap
...
for i = 1:(cutplane-1)
    ...
    ri = (cn-cr)/(mr-mn) - inlet_rad_factor*alpha;
...
% Save the files
Write_Plot3D('mesh.plot3d_step',Mesh);
Write_Boundata_File('p3d.boundata_step',boundata_new);